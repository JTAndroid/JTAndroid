...
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        if (!handleException(ex) && mDefaultHandler != null) {
    		...
    		try {
    			ActivityManager manager = (ActivityManager) App.getApplicationConxt().getSystemService(mContext.ACTIVITY_SERVICE);
    			...
    			for (int i = 0; i < runningTaskInfos.size(); i++) {
    				...
    				// 应用程序位于堆栈的顶层
    				if (mContext.getPackageName().equals(nowTask.topActivity.getPackageName())) {
    					myTask = nowTask;
    					break;
    				}
    			}
    			if (myTask != null) {
    				...
    				String mainName = SplashActivity.class.getName();
