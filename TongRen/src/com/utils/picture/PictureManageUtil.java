...
public class PictureManageUtil {
	 ...
	 *            旋转参数
	 * @return */
	public static Bitmap resizeBitmap(Bitmap bm, int newWidth, int newHeight, int rotate) {
		int width = bm.getWidth();
		int height = bm.getHeight();
		// 计算缩放比例
		float scaleWidth = ((float) newWidth) / width;
	 ...
	 * @param imagePath
	 * @return */
	public static int getPhotoOrientation(String absoluteImagePath) {
		int rotate = 0;
		try {
			ExifInterface exif = new ExifInterface(absoluteImagePath);
			int orientation = exif.getAttributeInt(ExifInterface.TAG_ORIENTATION, ExifInterface.ORIENTATION_NORMAL);
	...
	public static Bitmap cropBitmap(Bitmap bm) {
		...
		int width = bm.getWidth();
		if (height > width) {
			return Bitmap.createBitmap(bm, 0, (height - width) / 4, width, width);
		}
		else {
			return Bitmap.createBitmap(bm, (width - height) / 2, 0, height, height);
		}
	}
	...
	public static Bitmap getMicroImage(String filePath, int width) {
		...
		try {
			...
			bitmap = PictureManageUtil.resizeBitmap(bitmap, width, height, rotate);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (is != null) {
