...
public class AvatarUploaderConnection {

	private final String TAG = getClass().getSimpleName();
	
	// 常量
	private final String WEB_URL = EAPIConsts.AVATAR_CONNS_URL; // 服务器地址
	 ...
	 * @param listener
	 */
	public AvatarUploaderConnection(OnFileUploadListener listener){
		...
		new Thread(new Runnable(){
			...
			public void run() {
				...
				try {
					...
					params.put("fileWidth", mFileWidth + ""); // 图片尺寸
					...
					if(res == 200){
						...
						if(jObject.has(str_key) && !jObject.isNull(str_key)){
							...
							String returnnetUrl=subJObject.optString("relativelyUrl");//相对路径
							if(result){
								mStatus = Status.Success;
								if (mListener != null) {
									mListener.onSuccess(returnnetUrl);
								}
								is.close();
								return;
							}
