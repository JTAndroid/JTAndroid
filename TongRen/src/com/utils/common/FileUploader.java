...
public class FileUploader{
	...
	private OnMeetingFileUploadListener mMeetingFileListener;
	private OnMeetingPicUploadListener mMeetingPicListener;
	 ...
	 * @param listener
	 */
	public FileUploader(JTFile jtfile, OnMeetingFileUploadListener listener){
		
		// 准备就绪
		mStatus = Status.Prepared;
		if (mMeetingFileListener != null) {
			mMeetingFileListener.onPrepared();
		}
		// 初始化变量
		mIsCancel = false;
		mMeetingFileListener = listener;
		mJTFile = jtfile;
	}
	/**
	 * 会议文件 上传    构造函数
	 * @param listener
	 */
	public FileUploader(JTFile jtfile, OnMeetingPicUploadListener listener){
		
		// 准备就绪
		mStatus = Status.Prepared;
		if (mMeetingPicListener != null) {
			mMeetingPicListener.onPrepared();
		}
		// 初始化变量
		mIsCancel = false;
		mMeetingPicListener = listener;
		mJTFile = jtfile;
	}
	...
	public void start(){
		...
		if (!file.exists()) {
			...
			if(mListenerEx != null){
				mListenerEx.onError(mJTFile.mCreateTime, ErrorCode.FileNotExist, ErrorMessage.FILE_NOT_EXIST);
			}
			//会议上传文件不存在返回error
			if (mMeetingFileListener !=null) {
				mMeetingFileListener.onError(ErrorCode.FileNotExist, ErrorMessage.FILE_NOT_EXIST);
			}
			if (mMeetingPicListener !=null) {
				mMeetingPicListener.onError(ErrorCode.FileNotExist, ErrorMessage.FILE_NOT_EXIST);
			}
		...
		if (mMeetingFileListener!=null) {
			mMeetingFileListener.onStarted();
		}
		if (mMeetingPicListener!=null) {
			mMeetingPicListener.onStarted();
		}
	}
	...
	protected void doInBackgroundEx() {
		...
		new Thread(new Runnable(){
			...
			public void run() {
				...
				HashMap<String,String> params = new HashMap<String,String>();
				params.put("taskId", mJTFile.mTaskId);
				params.put("moduleType", mJTFile.mModuleType + "");//特殊：会议的moduleType为-1
				if(mJTFile.mModuleType==JTFile.TYPE_DEMAND) //如果是上传的需求附件  就添加
					params.put("fileType", mJTFile.getmType()+"");
				if (mJTFile.mModuleType==-1) //如果是会议
				...
				try {
					...
					if (mJTFile.mModuleType==-1) //如果是会议
						paramsEx.put("meetingId", mJTFile.reserved3);
					...
					while ((count = fis.read(buffer)) != -1) {
						...
						if (mMeetingFileListener!=null) {
							mMeetingFileListener.onUpdate(mProgress);
						}
						if (mMeetingPicListener!=null) {
							mMeetingPicListener.onUpdate(mProgress);
						}
						// Task被取消了，退出循环
						if (mIsCancel) {
							fis.close();
							...
							if (mMeetingFileListener!=null) {
								mMeetingFileListener.onCanceled();
							}
							if (mMeetingPicListener!=null) {
								mMeetingPicListener.onCanceled();
							}
							break;
						}
					}
							...
							if (result) {
								...
								if(subJObject.has(str_key) && !subJObject.isNull(str_key)){
									...
									if (mMeetingFileListener!=null) {
										//重新封装JTFile2ForHY
										Gson gson = new Gson();
										JTFile2ForHY jtFile2ForHY = gson.fromJson(subJObject.optString(str_key), JTFile2ForHY.class);
										mMeetingFileListener.onSuccess(jtFile2ForHY.fileIndexId+"", jtFile2ForHY);
									}
								}
								...
								if (subJObject.has(str_key) && !subJObject.isNull(str_key)) {
									if (mMeetingPicListener!=null) {
										Gson gson = new Gson();
										MMeetingPic mMeetingPic = gson.fromJson(subJObject.optString(str_key), MMeetingPic.class);
										mMeetingPicListener.onSuccess(mMeetingPic.getPicPath(), mMeetingPic);
									}
								}
								is.close();
								return;
								...
								if(mListenerEx != null){
									mListenerEx.onError(mJTFile.mCreateTime, ErrorCode.UnknownError, ErrorMessage.UNKNOWN_ERROR);
								}
								if (mMeetingFileListener!=null) {
									mMeetingFileListener.onError(ErrorCode.UnknownError, ErrorMessage.UNKNOWN_ERROR);
								}
								if (mMeetingPicListener!=null) {
									mMeetingPicListener.onError(ErrorCode.UnknownError, ErrorMessage.UNKNOWN_ERROR);
								}
							}
						}
					}
					...
					if(mListenerEx != null){
						mListenerEx.onError(mJTFile.mCreateTime, ErrorCode.NetworkError,ErrorMessage.NETWORK_FAILED);
					}
					if (mMeetingFileListener!=null) {
						mMeetingFileListener.onError(ErrorCode.NetworkError,ErrorMessage.NETWORK_FAILED);
					}
					if (mMeetingPicListener!=null) {
						mMeetingPicListener.onError(ErrorCode.NetworkError,ErrorMessage.NETWORK_FAILED);
					}
				} 
				catch (Exception e) {
					mStatus = Status.Error;
	...
	public interface OnFileUploadListenerEx {
		...
		public void onError(long uid, int code, String message); // 上传失败
	}
	/**
	 * 会议文件上传监听器（包含附件、视频等文件）
	 * @author zhongshan
	 *
	 */
	public interface OnMeetingFileUploadListener{
		public void onPrepared(); // 准备就绪
		public void onStarted(); // 开始任务
		public void onUpdate(int value); // 进度更新
		public void onCanceled(); // 取消上传
		public void onSuccess(String id,JTFile2ForHY jtfile); // 上传成功:会议返回文件id和文件file
		public void onError(int code, String message); // 上传失败
	}
	/**
	 * 会议文件上传监听器（包含图片）
	 ...
	 *
	 */
	public interface OnMeetingPicUploadListener{
		public void onPrepared(); // 准备就绪
		public void onStarted(); // 开始任务
		public void onUpdate(int value); // 进度更新
		public void onCanceled(); // 取消上传
		public void onSuccess(String url,MMeetingPic fileImg); // 上传成功:会议返回文件id和文件file
		public void onError(int code, String message); // 上传失败
	}
