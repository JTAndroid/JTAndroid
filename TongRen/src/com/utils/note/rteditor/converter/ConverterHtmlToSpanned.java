...
import com.utils.note.rteditor.api.format.RTSpanned;
import com.utils.note.rteditor.api.media.RTAudio;
import com.utils.note.rteditor.api.media.RTAudioImage;
import com.utils.note.rteditor.api.media.RTImage;
import com.utils.note.rteditor.api.media.RTVideo;
import com.utils.note.rteditor.converter.tagsoup.HTMLSchema;
import com.utils.note.rteditor.converter.tagsoup.Parser;
import com.utils.note.rteditor.effects.LeadingMarginEffect;
...
public class ConverterHtmlToSpanned implements ContentHandler {
    ...
    private void startAudio(Attributes attributes) {
        ...
        try {
            ...
            RTAudioImage image = new RTAudioImage(RTApi.getApplicationContext(),name,time);
            String imagePath = image.getPath();
            player.release();
            RTAudio audio = mMediaFactory.createAudio(imagePath, src);

            if (audio != null && audio.exists()) {
                String path = audio.getFilePath(RTFormat.SPANNED);
                File file = new File(path);
                if (file.isDirectory()) {
                    // there were crashes when an image was a directory all of a sudden...
                    // the root cause is unknown and this is a desparate work around
                ...
                // see http://www.fileformat.info/info/unicode/char/fffc/index.htm
                mResult.append("\uFFFC");
                AudioSpan audioSpan = new AudioSpan(audio, true);
                mResult.setSpan(audioSpan, len, len + 1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
