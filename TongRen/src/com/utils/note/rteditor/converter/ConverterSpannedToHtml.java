...
import com.utils.note.rteditor.spans.VideoSpan;
    ...
    private void convertParagraphs() {
        ...
        for (Paragraph paragraph : rtLayout.getParagraphs()) {
            ...
            if (alignmentType != null) {
            	String startTag = alignmentType.getStartTag();
            	if(startTag.startsWith("<div align")){
            		startTag = startTag.substring(0,startTag.length() - 1)
            				+ " start = \"" + paragraph.start()
            				+ "\" length = \"" + (paragraph.end()-paragraph.start()) + "\">";
            	}
                mOut.append(startTag);
            }
    ...
    private void convertText(Spanned text, int start, int end, SortedSet<CharacterStyle> spans) {
        while (start < end) {
            ...
            if (start < spanStart) {
                ...
                if(mIsJson){
//                    mOut.append(text.subSequence(start, Math.min(end, spanStart)));
                ...
                spans.remove(span);
if(mIsJson){
    if(handleJsonText(span,start,spanEnd - spanStart)){
    ...
    private boolean handleStartTag(CharacterStyle style) {
	    ...
	    /* Examples for fonts styles:
		   ...
		   <font style="font-size:50px;background-color:#0000FF;color:#FFFF00">This is heading 2</font> */
            ...
            mOut.append("<embed src=\"" + span.getAudio().getAudioPath() + "\">");
