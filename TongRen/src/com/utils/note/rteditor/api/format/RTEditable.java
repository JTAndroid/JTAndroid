...
package com.utils.note.rteditor.api.format;
...
import com.utils.note.rteditor.converter.ConverterSpannedToHtml;
import com.utils.note.rteditor.converter.ConverterSpannedToJson;
...
public final class RTEditable extends RTSpanned {
    ...
    public RTText convertTo(RTFormat destFormat, RTMediaFactory<RTImage, RTAudio, RTVideo> mediaFactory) {
        if(destFormat instanceof RTFormat.Json){
            clean();
            return new ConverterSpannedToJson().convert(mEditor.getText(), (RTFormat.Html) destFormat);

        } else if (destFormat instanceof RTFormat.Html) {
            clean();
