...
public class RTLayout implements Serializable {
    ...
    public int getLineForOffset(int offset) {
        int lineNr = 0;
        while (lineNr < mNrOfLines && mParagraphs.size() > 0 && lineNr< mParagraphs.size() && offset >= mParagraphs.get(lineNr).end()) {
            lineNr++;
        }
    ...
    public int getLineStart(int line) {
        if(mNrOfLines == 0 || line < 0){
            return 0;
        }else{
            if(line < mNrOfLines){
                line = line >= mParagraphs.size() ? mParagraphs.size() - 1 : line;
                return mParagraphs.get(line).start();
            }else{
                return mParagraphs.get((mNrOfLines - 1) >= mParagraphs.size() ? mParagraphs.size() - 1 : mNrOfLines - 1).end() - 1;
            }
        }
    }
    ...
    public int getLineEnd(int line) {
        if(mNrOfLines == 0 || line < 0){
            ...
            if(line < mNrOfLines){
                line = line >= mParagraphs.size() ? mParagraphs.size() - 1 : line;
                return mParagraphs.get(line).end();
            }else{
                return mParagraphs.get((mNrOfLines - 1) >= mParagraphs.size() ? mParagraphs.size() - 1 : mNrOfLines - 1).end() - 1;
            }
        }
