 ...
 */
package com.tr.ui.widgets.floatmeune;
...
public class FloatingActionMenu {
	...
	public void notifiyAngle(int startAngle, int endAngle) {
		this.startAngle = startAngle;
		this.endAngle = endAngle;
	}
	 ...
	 * @param animated
	 */
	public FloatingActionMenu(final View mainActionView, int startAngle, int endAngle, int radius, List<Item> subActionItems, MenuAnimationHandler animationHandler, boolean animated,
		...
		this.mainActionView.setClickable(false );
	...
	private Point getActionViewCoordinates() {
		...
		if (systemOverlay) {
			...
			if (hasSmartBar()) {
				coords[1] -= (activityFrame.height() + activityFrame.top - getActivityContentView().getMeasuredHeight()-1.8*getStatusBarHeight());
			}else {
