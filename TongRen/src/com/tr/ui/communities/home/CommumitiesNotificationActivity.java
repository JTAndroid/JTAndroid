...
public class CommumitiesNotificationActivity extends JBaseActivity implements IBindData{
	...
	private void initData() {
		communityNotifylist = (ArrayList<CommunityNotify>) getIntent().getSerializableExtra("communityNotifylist");
		adapter = new CommumitiesNotificationAdapter(this);
		if(communityNotifylist!=null){
			if(communityNotifylist.size()>0){
				...
				CommunityReqUtil.doGetCommunityListByUserId(CommumitiesNotificationActivity.this, CommumitiesNotificationActivity.this, App.getUserID(), null);
			}
		}else{
			NoContent.setVisibility(View.VISIBLE);
			CommunityReqUtil.doGetCommunityListByUserId(CommumitiesNotificationActivity.this, CommumitiesNotificationActivity.this, App.getUserID(), null);
	...
	private void initView() {
		...
		commumitiesnotificationLv = (XListView) findViewById(R.id.commumitiesnotificationLv);
...
//					communityNotifylist.get(mIndex).setAcceptStatus(acceptStatus);
		...
		case EAPIConsts.CommunityReqType.TYPE_ASSIGNMENT_COMUNITY:
			...
			if (isResponse) {
				showToast("转让成功");
				CommunityReqUtil.doHandleApply(this, this, mCommunityNotify.getId(), 1, new Date().getTime(), null);
			}else{
				showToast("转让失败");
			}
			break;
		...
		public void handleMessage(android.os.Message msg) {
			...
			case EAPIConsts.handler.show_err:
				...
				if(errCode.equals("-1")){
					...
					CommunityReqUtil.doHandleApply(CommumitiesNotificationActivity.this, CommumitiesNotificationActivity.this, mCommunityNotify.getId(), 1, new Date().getTime(), null);
				}
				break;
			}
		};
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			notice_item_refuse.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					CommunityReqUtil.doHandleApply(mContext, CommumitiesNotificationActivity.this, communityNotify.getId(), 2, new Date().getTime(), null);
				...
				public void onClick(View v) {
					...
					case 0://0加群 
						...
						case 0:
							...
							CommunityReqUtil.doInvite2Muc(mContext, communityNotify.getCommunityId(), list, CommumitiesNotificationActivity.this, mHandler);
							break;
						case 1:
						...
						break;
					case 1:
						CommunityReqUtil.doAssignmentCommunity(mContext, communityNotify.getCommunityId()+"", communityNotify.getApplicantId()+"", CommumitiesNotificationActivity.this, mHandler);
						break;
					}
