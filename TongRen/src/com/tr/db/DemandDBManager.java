 ...
 *
 */
public class DemandDBManager {

	// 常量
	private final String TAG = getClass().getSimpleName();

	// 变量
	private DemandDBHelper helper;
	private SQLiteDatabase db;

	public DemandDBManager(Context context) {
		helper = DemandDBHelper.getInstance(context);
		synchronized (helper) {
	...
	public List<Metadata> queryInvestType(String id) {
		...
		synchronized (helper) {
			...
			Cursor c = db.rawQuery("select * from tb_code where type= ?",
					new String[] { id });
			try {
				while (c.moveToNext()) {
